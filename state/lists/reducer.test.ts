...
import AONSWAP_DEFAULT_TOKEN_LIST from '@aonswap/default-token-list'
  ...
  describe('updateVersion', () => {
    describe('never initialized', () => {
      beforeEach(() => {
        store = createStore(reducer, {
          byUrl: {
            'https://unpkg.com/@aonswap/default-token-list@latest/aonswap-default.tokenlist.json': {
              error: null,
              current: STUB_TOKEN_LIST,
              loadingRequestId: null,
              pendingUpdate: null
            },
            'https://unpkg.com/@aonswap/default-token-list@latest': {
              error: null,
              current: STUB_TOKEN_LIST,
              loadingRequestId: null,
              pendingUpdate: null
      ...
      it('clears the current lists', () => {
        expect(
          store.getState().byUrl['https://unpkg.com/@aonswap/default-token-list@latest/aonswap-default.tokenlist.json']
        ).toBeUndefined()
        expect(store.getState().byUrl['https://unpkg.com/@aonswap/default-token-list@latest']).toBeUndefined()
      })
        ...
        Object.keys(s.byUrl).forEach(url => {
          if (url === DEFAULT_TOKEN_LIST_URL) {
            expect(s.byUrl[url]).toEqual({
              error: null,
              current: aonSWAP_DEFAULT_TOKEN_LIST,
              loadingRequestId: null,
              pendingUpdate: null
            })
          } else {
    ...
    describe('initialized with a different set of lists', () => {
      beforeEach(() => {
        store = createStore(reducer, {
          ...
          selectedListUrl: undefined,
          lastInitializedDefaultListOfLists: ['https://unpkg.com/@aonswap/default-token-list@latest']
        })
        store.dispatch(updateVersion())
      })
      ...
      it('each of those initialized lists is empty', () => {
        ...
        // note we don't expect the aonswap default list to be prepopulated
        // this is ok.
        Object.keys(byUrl).forEach(url => {
          if (url !== 'https://unpkg.com/@aonswap/default-token-list@latest/aonswap-default.tokenlist.json') {
